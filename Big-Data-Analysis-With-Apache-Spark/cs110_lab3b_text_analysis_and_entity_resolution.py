# Databricks notebook source exported at Fri, 16 Dec 2016 23:05:01 UTC
# MAGIC %md
...
# TODO: Replace <FILL IN> with appropriate code
amazonRecToToken = amazonSmall.map(lambda a:(a[0],tokenize(a[1])))
googleRecToToken = googleSmall.map(lambda a:(a[0],tokenize(a[1])))
...
def countTokens(vendorRDD):
    ...
    """
    return vendorRDD.map(lambda a: len(a[1])).reduce(lambda a,b:a+b)
...
def idfs(corpus):
    ...
    """
    uniqueTokens = corpus.map(lambda a:set(a[1]))
    tokenCountPairTuple = uniqueTokens.map(lambda a: [(i,1) for i in a])
    tokenSumPairTuple = tokenCountPairTuple.reduce(lambda a,b:a+b)
    N = corpus.count()
    return (sc.parallelize(tokenSumPairTuple).reduceByKey(lambda a,b:(a+b)).map(lambda a:(a[0],float(N)/a[1])).cache())
...
# TODO
smallIDFTokens = idfsSmall.takeOrdered(11,lambda a:a[1])
...
idfsToCountRow = ['token','count']
idfsToCountDF = sqlContext.createDataFrame(idfsSmall,idfsToCountRow)
...
def tfidf(tokens, idfs):
    ...
    """
    tfs = tf(tokens)
    tfIdfDict={k:tfs[k]*idfs[k] for k in tokens}
...
def fastCosineSimilarity(record):
    ...
    value = float(s)/amazonNormsBroadcast.value[amazonRec]/googleNormsBroadcast.value[googleRec]
