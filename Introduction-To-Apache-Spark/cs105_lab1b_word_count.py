...
pluralDF = wordsDF.select(concat('word',lit('s')).alias('words'))
pluralDF.show()
...
pluralLengthsDF = pluralDF.select(length('words').alias('length'))
...
def removePunctuation(column):
    ...
    """
    return trim(lower(regexp_replace(column,'([^\w ]+)',''))).alias('changed')
...
shakeWordsDF = (shakespeareDF.select(explode(split('changed',' ')).alias('word')).where("word !=''")
                )
shakeWordsDF.show()
