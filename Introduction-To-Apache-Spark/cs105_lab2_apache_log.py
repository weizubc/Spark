# Databricks notebook source exported at Fri, 16 Dec 2016 23:04:17 UTC
# MAGIC %md
...
day_counts = (daily_hosts_df
                .select('day', 'count')
                .map(lambda r: (r[0], r[1]))
                .collect())

days_with_hosts, hosts = zip(*day_counts)
...
avg_daily_req_per_host_df = (
  ...
  .select(daily_hosts_df['day'], 
          sqlFunctions.floor(total_req_per_day_df['count']/daily_hosts_df['count'])
...
day_avgs = (avg_daily_req_per_host_df
                .map(lambda r: (r[0], r[1]))
                .collect())

days_with_avg, avgs = zip(*day_avgs)
...
not_found_df = logs_df.select('*').filter(logs_df['status'] ==404).cache()
...
errors_by_date_sorted_df = not_found_df.select(dayofmonth('time').alias('day')).groupby('day').count().sort('day').cache()
...
# TODO: Replace <FILL IN> with appropriate code
hour_count=hour_records_sorted_df.map(lambda r: (r[0], r[1])).collect()
hours_with_not_found,not_found_counts_per_hour = zip(*hour_count)
